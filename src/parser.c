...
ast_node *parse_cmp(parser_state *s) {
  ast_node *left = parse_addsub(s);
  if (left == NULL) {
    return NULL;
  }

  while ((s->current_kind == 3 && ((*s->src == '!' && *(s->src + 1) == '=') ||
                                   *s->src == '<' || *s->src == '>')) ||
         (*s->src == '=' && *(s->src + 1) == '=')) {
    ast_node *new = malloc(sizeof(ast_node));
    if (new == NULL) {
...
ast_node *parse_statement(parser_state *s, ast_node_function *parent_func) {
  ...
  if (s->current_len == 6 && strncmp(s->src, "return", 6) == 0) {
    ...
    advance(s); // =
    if (s->current_kind != 4) {
      variable->initializer = parse_cmp(s);
    }
    node->node = variable;
  }
